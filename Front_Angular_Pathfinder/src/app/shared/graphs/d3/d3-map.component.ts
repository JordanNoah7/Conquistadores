...
@Component({
  selector: 'd3-map',
  template: ` <div class="d3-zoomin" (click)="zoomIn()">
      <i class="fa fa-plus"></i>
    </div>
    <div class="d3-zoomout" (click)="zoomOut()">
...
export class D3MapComponent implements AfterViewInit {
  ...
  d3Init() {
    ...
    d3.json(
      config.API_URL + '/graphs/d3/world-topo-min.json',
      (err: any, world: any) => {
        world.objects.countries.geometries =
          world.objects.countries.geometries.map((it: any) => {
  ...
  handleMouseOver(country: any) {
    const mouse = d3.mouse(this.svg.node()).map((d: any) => {
      return parseInt(d);
    });
  ...
  isVisible() {
    ...
    return container.clientHeight > 0 && container.clientWidth > 0;
  }
  ...
  throttleTimer: number | undefined;
  ...
  addpoint(lon: any, lat: any, text: any) {
